...
(function() {
    ...
    var bendFn = function() {
        ...
        var restoreActiveUser = function(options) {
            ...
            return promise.then(function(user) {
                ...
                return Bend.User.me(options).then(function(response) {
                    ...
                    if(Bend.Error.INVALID_CREDENTIALS === error.name) {
         ...
         * @throws {Bend.Error} user argument must contain: _id, _bmd.authtoken.
         */
         ...
         *                          `masterSecret`.
         * @throws {Bend.Error} Bend requires https as the protocol when setting Bend.APIHostName
        ...
        Bend.init = function(options) {
            ...
            var apiHostName = options.apiHostName || Bend.API_ENDPOINT;
            Bend.APIHostName = apiHostName || Bend.APIHostName;

            // Check if Bend.APIHostName uses https protocol
            /*
             if(Bend.APIHostName.indexOf('https://') !== 0) {
             throw new Bend.Error('Bend requires https as the protocol when setting' +
             ' Bend.APIHostName, instead found the protocol ' +
             Bend.APIHostName.substring(0, Bend.APIHostName.indexOf(':/')) +
             ' in Bend.APIHostName: ' + Bend.APIHostName);
             }
             */

            // Set the Client App Version
            if(options.clientAppVersion != null) {
                Bend.ClientAppVersion.setVersion(options.clientAppVersion);
            }

            // Set the custom request properties
            if(options.customRequestProperties != null) {
                Bend.CustomRequestProperties.setProperties(options.customRequestProperties);
            }
        ...
        var Auth = /** @lends Auth */ {
            ...
            Session: function() {
                ...
                if(null === user) {
                    error = clientError(Bend.Error.NO_ACTIVE_USER);
                    return Bend.Defer.reject(error);
                }

                // Check if user has property _bmd
                if(user._bmd === null || user._bmd === undefined) {
                    error = new Bend.Error('The active user does not have _bmd defined as a property.' +
        ...
        /**
         * @memberof! <global>
         * @namespace Bend.ClientAppVersion
         */
        Bend.ClientAppVersion = /** @lends Bend.ClientAppVersion */ {
        ...
        /**
         * @memberof! <global>
         * @namespace Bend.CustomRequestProperties
         */
        Bend.CustomRequestProperties = /** @lends Bend.CustomRequestProperties */ {
            ...
            setProperties: function(properties) {
                Bend.CustomRequestProperties.clear();
                Bend.CustomRequestProperties.addProperties(properties);
            },
        ...
        Bend.DataStore = /** @lends Bend.DataStore */ {
            ...
            update: function(collection, document, options) {
                ...
                // Validate arguments.
                if(null == document._id) {
                    error = new Bend.Error('document argument must contain: _id');
                    throw error;
                    // return Bend.Defer.reject(error);
                }
        ...
        Bend.Metadata.prototype = /** @lends Bend.Metadata# */ {
            ...
            getCreatedAt: function() {
                if(null != this._document._bmd && null != this._document._bmd.ect) {
                    return fromISO(this._document._bmd.ect);
                }
                return null;
            },
            ...
            getLastModified: function() {
                if(null != this._document._bmd && null != this._document._bmd.lmt) {
                    return fromISO(this._document._bmd.lmt);
                }
                return null;
            },
                ...
                var promise = Social[provider](options).then(function(tokens) {
                    ...
                    if(null !== activeUser) {
                        ...
                        if(activeUser._id == null) {
                            error = new Bend.Error('Active user does not have _id property defined.');
                            throw error;
                        }

                        // Check user for property _id. Thrown error will reject promise.
                        if(user._id == null) {
                            error = new Bend.Error('User does not have _id property defined.');
                            throw error;
                        }
            ...
            logout: function(options) {
                ...
                if(options.silent && null === Bend.getActiveUser()) {
                    ...
                    promise = Bend.Persistence.create({
                        ...
                        // Check if previous has property _bmd. Thrown error will cause promise to be
                        // rejected
                        if(previous._bmd == null) {
                            error = new Bend.Error('The previous active user does not have _bmd defined' +
                                'as a property.');
                            throw error;
                        }
        ...
        var BendReference = /** @lends BendReference */ {
            ...
            get: function(document, options) {
                ...
                if(isArray(document)) {
                    var promises = document.map(function(member) {
                        return BendReference.get(member, clone(options));
                    });
                ...
                var resolveRelationships = function(entity, relationMapping) {
                    ...
                    if(relationMapping.keys) {
                        ...
                        Object.keys(relationMapping.keys).forEach(function(key) {
                            ...
                            if(relationLevel.resolve) {
                                ...
                                var promises = (isKeyArray ? entity[key] : [entity[key]]).map(function(member) {
                                    ...
                                    if(member._id == null) {
                                        error = new Bend.Error('Member does not have _id property defined. ' +
                        ...
                        return Bend.Defer.all(relationshipPromises);
                    }
                    else {
                        return Bend.Defer.resolve();
                    }
            ...
            save: function(collection, document, options) {
                ...
                if(isArray(document)) {
                    var promises = document.map(function(member) {
                        return BendReference.save(collection, member, clone(options));
                    });
                    return Bend.Defer.all(promises);
                }
        ...
        var maxAge = {
             ...
             * @throws {Bend.Error} The item does not have _bmd defined as a property.
            ...
            status: function(data, maxAge) {
                ...
                for(var i = 0; i < length; i += 1) {
                    ...
                    // Check if item has property _bmd
                    if(item._bmd == null) {
                        var error = new Bend.Error('The item does not have _bmd defined as a property.' +
                            'It is required to get the maxAge status.');
                        throw error;
                    }
            ...
            read: function(request, options) {
                ...
                if('_me' === request.collection) { // Me.
                    ...
                    if(null !== user) {
                        ...
                        if(user._id == null) {
                            error = new Bend.Error('Active user does not have the _id property defined. ' +
        ...
        Bend.Persistence.Net = /** @lends Bend.Persistence.Net */ {
            ...
            _request: function(request, options) {
                ...
                // Format: <APIHostName>/<namespace>[/<Bend.appKey>][/<collection>][/<id>]
                ...
                var url = [Bend.APIHostName].concat(segments).join('/') + '/';
                ...
                options.clientAppVersion = options.clientAppVersion || Bend.ClientAppVersion.stringValue();
                if(options.clientAppVersion != null) {
                    headers['X-Bend-Client-App-Version'] = (options.clientAppVersion + '');
                }
                ...
                var customRequestProperties = Bend.CustomRequestProperties.properties();
                ...
                // Set X-Bend-Custom-Request-Properties to the JSON string of the custom
                ...
                if(customRequestPropertiesByteCount >= CRP_MAX_BYTES) {
                    throw new Bend.Error('Custom request properties is ' + customRequestPropertiesByteCount +
                        '. It must be less then ' + CRP_MAX_BYTES + ' bytes.');
                }

                // Set the custom request properties header.
                /*
                headers['X-Bend-Custom-Request-Properties'] = customRequestPropertiesHeader;
                */
                        ...
                        options
                            ...
                            if(request.method === 'GET' &&
                                ...
                                if(isArray(response) && expectSingleEntity) {
                                    error = new Bend.Error('Expected a single entity as a response to ' +
                                        request.method + ' ' + url + '. Received an array ' +
                                        'of entities instead.');
                                    throw error;
                                }
                                else if(!isArray(response) && !expectSingleEntity) {
                                    error = new Bend.Error('Expected an array of entities as a response to ' +
        ...
        var Sync = /** @lends Sync */ {
                ...
                return Database.findAndModify(Sync.system, collection, function(metadata) {
                    ...
                    documents.forEach(function(document) {
                        ...
                        if(document._id == null) {
                            error = new Bend.Error('Document does not have _id property defined. ' +
                        ...
                        var timestamp = null != document._bmd ? document._bmd.lmt : null;
                        var clientAppVersion = options.clientAppVersion || Bend.ClientAppVersion.stringValue(),
                            customRequestProperties = options.customRequestProperties || {};

                        // Get globally set custom request properties.
                        var globalCustomRequestProperties = Bend.CustomRequestProperties.properties();
            ...
            _read: function(collection, documents, options) {
                ...
                var promises = identifiers.map(function(id) {
                    ...
                    var request = {
                        ...
                        query: new Bend.Query().contains('_id', [id]),
                        auth: Auth.Default
                    };

                    // Read from local and net in parallel.
                    return Bend.Defer.all([
                        Bend.Persistence.Local.read(request, requestOptions),
                        Bend.Persistence.Net.read(request, requestOptions)
                    ]).then(function(responses) {
                        return {
                            local: responses[0],
                            net: responses[1]
            ...
            _destroy: function(collection, documents, options) {
                var promises = documents.map(function(composite) {
                    ...
                    var promises = [
                        Bend.Persistence.Local.destroy(request, requestOptions),
                        Bend.Persistence.Net.destroy(request, requestOptions)
                    ];
            ...
            _document: function(collection, metadata, local, net, options) {
                ...
                if(metadata == null) {
                    error = new Bend.Error('Missing required metadata for the document in collection ' +
                ...
                if(metadata.id == null) {
                    error = new Bend.Error('Metadata does not have id defined. This is ' +
                ...
                if(net != null) {
                    // Check if net has property _bmd
                    if(net._bmd == null) {
                        error = new Bend.Error('The server entity does not have _bmd defined as a property. ' +
                            'This is required to properly sync server entity _id ' +
                            net._id + ' in collection ' + collection + '.');
                        return Bend.Defer.reject(error);
                    }

                    // Check if net has property _bmd.lmt.
                    if(net._bmd.lmt == null) {
                        error = new Bend.Error('The server entity does not have _bmd.lmt defined as a ' +
                ...
                // Reject otherwise.
                if(null === net || (null != net._bmd && metadata.timestamp === net._bmd.lmt)) {
                    return Bend.Defer.resolve({
                        id: metadata.id,
                        document: local,
                        metadata: metadata
        ...
        // Use `promiscuous` as `Bend.Defer` adapter.
        ...
        // `Bend.Persistence.Net` adapter for [XHR](http://www.w3.org/TR/XMLHttpRequest2/).
        var Xhr = {
            /**
            ...
            /**
             * @augments {Bend.Persistence.Net.base64}
             */
            ...
            request: function(method, url, body, headers, options) {
                ...
                if(0 === url.indexOf(Bend.APIHostName) && 'GET' === method) {
                ...
                request.onabort = request.onerror = request.onload = request.ontimeout = function(event) {
                    ...
                    /*
                     ...
                     error = new Bend.Error('Content-Type header missing in response. Please add ' +
                     'Content-Type header to response with value ' +
                     'application/json.');
                     }
                     else if(responseContentType.indexOf('application/json') === -1) {
                     error = new Bend.Error('Response Content-Type header is set to ' +
