...
class FilesBlockItem extends Component {
    ...
    renderStatic() {
        ...
        var thumbnailURL = (fitImage ? _.get(blob,'_versions.thumbnail-fit._downloadURL') : _.get(blob,'_versions.thumbnail._downloadURL')) || _.get(blob,'_downloadURL');
    ...
    renderEditable() {
        const { name, isDragging, connectDragSource, connectDropTarget, connectDragPreview, uploader, filename, filesize, fitImage } = this.props;
        var blob = this.props._blob || this.props.blob;
        ...
        return connectDropTarget(connectDragPreview(
            <div className={classes("file-item is-editor",isDragging ? 'transparent' : '')}>
                {connectDragSource(<div className={classes("preview",previewIconClass(filename))} style={previewStyle}>
                ...
                <div className="remove-button" style={closeStyle} onClick={this.handleRemove.bind(this)}></div>
...
class FileListItem extends Component {
    ...
    renderEditable() {
        const { name, isDragging, connectDragSource, connectDropTarget, connectDragPreview, filename, filesize, description } = this.props;
        ...
        return connectDropTarget(connectDragPreview(
            <div className={classes("file-list-item is-editor",isDragging ? 'transparent' : '')}>
                ...
                <div>
                    <div className="content-container">
                        <div className="title"><ContentEditable html={name} onChange={this.handleNameChange.bind(this)}/></div>
                        <div className="description"><ContentEditable html={description} onChange={this.handleDescriptionChange.bind(this)}/></div>
                    </div>
                </div>
                {connectDragSource(<div className="drag-handler"></div>)}
                <div className="remove-button" onClick={this.handleRemove.bind(this)}></div>
            </div>
        ))
...
export default class FilesBlock extends BaseBlock {
    ...
    componentDidMount() {
        ...
        BendUtils.wrapInCallback(Bend.File.find(query),(err,blobs) => {
            ...
            var filesToProcess = _(files).filter((file) => {
                return _.find(blobs,(blob) => {
                    return blob._id === file.id;
                })
            }).value();
    ...
    removeFile(id) {
        var newState = {
            files: _.filter(this.state.files,(file) => { return file.id!==id})
        };
    ...
    addFiles(files) {
        ...
        this.setState({
            files: this.state.files.concat(_.map(files,(file) => {
                var id = Guid.raw();
                var findFile = () => {
                    return _.find(this.state.files,(file) => { return file.id === id });
                }
